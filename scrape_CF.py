...
def download_file (url, file_link, lang):
    ...
    if get_free_space_mb ("D:\TEMP") > 50000: 
        ...
        #print (file_num)
        #query whether it's already there
        #if :
        #  print(file_num + ' already loaded')
        #else:
        wav_dir='D:\\CallFriend\\' + lang + '\\'
...
def drill (url):
    ...
    for link in soup.find_all('a'):
        ...
        if file_link.find('?') == -1 and file_link[0] != '/':
            link_idx = (url + file_link).find('CallFriend') 
            if link_idx != -1:
                end_idx = (url + file_link)[link_idx + 11:].find('/')
                if end_idx != -1:
                    lang =  (url + file_link)[link_idx + 11:link_idx + 11 + end_idx]
                    #print(lang)
                    if file_link.find('.') == -1:
                        drill (url + file_link)
                    else:
...
pg1 = 'https://media.talkbank.org/ca/CallFriend/'
drill(pg1)
